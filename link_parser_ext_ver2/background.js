      ...
      setTimeout(() => {
        ...
        chrome.tabs.sendMessage(tab.id, { action: "getMeta" }, (response) => {
          ...
          const data = {
            ...
            keywords: !meta.keywords && !normalizedData['keywords'] ? [] : // Если оба значения отсутствуют, присваиваем пустой массив
                Array.isArray(meta.keywords) ? meta.keywords : // Если meta.keywords — массив, используем его
                    (meta.keywords || normalizedData['keywords']) // Иначе используем meta.keywords или normalizedData['keywords']
                        .split(',') // Разделяем строку по запятым
                        .map(keyword => keyword.trim()) // Убираем лишние пробелы
                        .filter(keyword => keyword !== ""), // Убираем пустые строки
